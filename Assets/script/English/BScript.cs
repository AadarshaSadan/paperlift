    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hitpoint))
        {
            ...
            if (hitpoint.collider.name != "wallpaper")
            {
                ...
                if (hitpoint.transform.gameObject.tag == "verticle" && firstorder)
                {

                    hitpoint.collider.GetComponent<MeshRenderer>().material.color = Color.red;
                }
                if (hitpoint.transform.gameObject.tag == "firsttrace" && secondorder)
                {

                    hitpoint.collider.GetComponent<MeshRenderer>().material.color = Color.red;
                }

                if (hitpoint.transform.gameObject.tag == "secondtrace" && thirdorder)
                {
